...
以底部操作栏轻量入口 + 弹窗管理方式接入 Reasonix Web UI：

- 底部操作栏有"特性"按钮，点击弹出特性管理弹窗
- 顶部分页标签栏，按状态分组（wishlist / planned / in_progress / custom_active / upstream_covered / dropped），每页显示对应条目数
- 搜索框支持实时过滤标题、简述和备注
- 每个条目以紧凑行布局展示：优先级徽标 + 标题（同行），可选的简述和备注在下方
- 点击条目直接进入编辑；编辑器中可修改所有字段，已有条目可删除（二次确认）
...
Web UI 中以标签栏（tabs）展示各状态，从左到右固定顺序为：
...
打开弹窗时自动选中第一个非空标签；手动切换标签后保持用户选择。每个标签旁显示该状态的条目数量。
...
1. 底部操作栏"特性"按钮，点击弹出弹窗
2. 标签栏按状态分组（wishlist → planned → in_progress → custom_active → upstream_covered → dropped），每个标签旁显示计数
3. 搜索框实时过滤标题 / 简述 / 备注
4. 新增 feature（顶部 + 图标按钮）
5. 编辑 feature（点击条目行直接进入编辑器）
6. 改 feature 状态（在编辑器中修改状态下拉框）
7. 删除 feature（编辑器中点击"删除此条目"，二次确认）
8. 紧凑行布局：优先级徽标 + 标题同行，简述/备注在下方
